...
export class HistorialProvider {
  ...
  agregar_historial(texto:string){
    ...
    this._historial.unshift(data); //este metodo agrega el elemento al inicio del arreglo, contrario al push..
  ...
  abrir_scan(index:number){
    ...
    switch(scanData.tipo){
      ...
      case "contacto":
        ...
        break;
      case "email":
        this.enviar_email(scanData.info);
        break;
      default:
  ...
  private enviar_email(texto:string){
    console.log(  texto);
    let emailArray = texto.split(";");
    console.log(emailArray);
    let emailDir = emailArray[0].replace("MATMSG:TO:", "");
  ...
  private crear_contacto(texto:string){
    ...
    if(!this.platform.is('cordova')){
      console.warn("En la pc no podemos crear el contacto");
      return;
    }
    ...
    contact.save().then(
      ...
      (err)=>this.crear_toast("Error: " + err)
    );
